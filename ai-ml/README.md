...
- **[LLM fine-tuning workflow](llm-fine-tuning-workflow/)** — a config-driven LoRA fine-tuning pipeline for small language models: data preparation, training, evaluation, and local inference, all runnable on a laptop CPU. Cuts validation perplexity from 41.4 to 22.8 on a 900-example instruction dataset.
